import { a as createAstro, c as createComponent, m as maybeRenderHead, b as renderTemplate } from './astro/server_8aaG9U9U.mjs';
import 'kleur/colors';
import 'clsx';

const $$Astro = createAstro("http://localhost:4321");
const $$TechSpecs = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$TechSpecs;
  const components = [
    {
      name: "CTA Banner",
  ...
  const techStack = [
    ...
    },
    {
      name: "AOS",
      version: "^2.3.4",
      description: "Animate On Scroll library for scroll animations"
    },
    {
      name: "Lucide Icons",
      version: "^0.473.0",
  ...
  return renderTemplate`${maybeRenderHead()}<section class="py-small"> <div class="site-container px-8"> <div class="space-y-12"> <div> <h2 class="text-large mb-6">Available Components</h2> <ul class="space-y-3"> ${components.map((component) => renderTemplate`<li> <code class="text-sm">${component.name}</code> <span class="mx-2 text-accent">—</span> <span class="text-sm text-body-base">${component.description}</span> </li>`)} </ul> </div> <div> <h2 class="text-large mb-6">Configured Collections</h2> <ul class="space-y-3"> ${collections.map((collection) => renderTemplate`<li> <code class="text-sm">${collection.name}</code> <span class="mx-2 text-accent">—</span> <span class="text-sm text-body-base">${collection.description}</span> </li>`)} </ul> </div> <div> <h2 class="text-large mb-6">Libraries & Dependencies</h2> <ul class="space-y-3"> ${techStack.map((tech) => renderTemplate`<li> <code class="text-sm">${tech.name}</code> <span class="mx-2 text-accent">—</span> <span class="text-sm text-body-base">${tech.description}</span> <span class="ml-2 text-xs text-body-base/80">${tech.version}</span> </li>`)} </ul> </div> <div> <h2 class="text-large mb-6">Technical Specifications</h2> <ul class="space-y-3"> ${specs.map((spec) => renderTemplate`<li> <code class="text-sm">${spec.name}</code> <span class="mx-2 text-accent">—</span> <span class="text-sm text-body-base">${spec.description}</span> </li>`)} </ul> </div> </div> </div> </section>`;
}, "/home/jose/Escritorio/jmf/JMF/src/pages/theme-info/TechSpecs.astro", void 0);

const $$file = "/home/jose/Escritorio/jmf/JMF/src/pages/theme-info/TechSpecs.astro";
const $$url = "/theme-info/TechSpecs";
...
export { $$TechSpecs as $, _page as _ };
